...
void get_defconfig_path(char *out, size_t bufsize, char *argv0)
{
	...
	struct stat info;
#ifdef WIN32
	snprintf(out, bufsize, "%s\\vrminer\\vrminer.conf\0", getenv("APPDATA"));
#else
	snprintf(out, bufsize, "%s\\.vrminer\\vrminer.conf", getenv("HOME"));
#endif
	if (dir && stat(out, &info) != 0) {
		// binary folder if not present in user folder
		snprintf(out, bufsize, "%s%svrminer.conf%s", dir, sep, "");
	}
	if (stat(out, &info) != 0) {
		out[0] = '\0';
		return;
...
bool stratum_handle_method(struct stratum_ctx *sctx, const char *s)
{
	...
	if (!strcasecmp(method, "client.get_algo")) { // vrminer only yet!
	...
	if (!strcasecmp(method, "client.get_stats")) { // vrminer/yiimp only yet!
